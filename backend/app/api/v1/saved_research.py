...
async def save_research(body: SaveResearchRequest, request: Request, user: UserContext = Depends(require_active_user)) -> dict:
    db = request.state.db
    # NOTE: research_gen quota belongs on the actual generation endpoint (expensive),
    # not on saving an existing report (cheap). Will be wired when research
    # generation gets its own endpoint. See quotas.py for the config.
